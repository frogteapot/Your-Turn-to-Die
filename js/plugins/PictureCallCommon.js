 ...
 *
 *  P_CALL_CE_2 [ピクチャ番号] [コモンイベントID] [トリガー] [透明色を考慮]:
 *
 *
...
(function() {
    'use strict';
    var pluginName2 = 'PictureCallCommon2';

    var getParamOther = function(paramNames) {
        if (!Array.isArray(paramNames)) paramNames = [paramNames];
        for (var i = 0; i < paramNames.length; i++) {
            var name = PluginManager.parameters(pluginName2)[paramNames[i]];
            if (name) return name;
        }
        return null;
    };
    ...
    Game_Map.prototype.setupPictureCommonEvent = function() {
        ...
        var event    = commonId > 1000 ? $dataCommonEvents2[commonId - 1000] : $dataCommonEvents[commonId];
        var result   = false;
    ...
    Sprite_Picture.moveCommonList = [9, 122, 310, 960, 963, 1063, 1087]; // 1a, 1b, 2a, 2b, 2b intro, 3a, 3a move scenes
    ...
    Sprite_Picture.prototype.callTouch = function() {
        ...
        for (var i = 0, n = this._triggerHandler.length; i < n; i++) {
            ...
            if (Sprite_Picture.moveCommonList.includes(commandIds[i]) && Input.isTriggered('shift'))
            {
