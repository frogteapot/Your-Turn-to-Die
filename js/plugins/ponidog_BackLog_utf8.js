...
for (i = 1; i < args.length; i++)
        name += " " + args[i];
oBackLog.sNameBoxSet_ponidogBackLog=name;//
        }
...
// Replace \N[4] with actor name
sNameSet = sNameSet.replace('\\N[4]', this.actorName(4));
...
//\x1bは前半に変換された＼である。
    text = text.replace(/\x1bC\[(\d+)\]/gi, ''); // originally replaced with \x1b, but this caused normal characters like ! and N to get eaten down below
...
// Remove MPP_MessageEX control codes
    text = text.replace(/\x1bsp\[(\d+)\]/g, '');
    text = text.replace(/\x1bsw\[(\d+)\]/g, '');
    text = text.replace(/\x1bse\[([^\]]*)\]/g, '');
    text = text.replace(/\x1bfs\[(\d+)\]/g, '');
    
// Others that I don't think actually get used, but just being safe
    text = text.replace(/\x1bat\[(\d+)\]/g, '');
    text = text.replace(/\x1bco\[(\d+)\]/g, '');
    text = text.replace(/\x1bpx\[(\d+)\]/g, '');
    text = text.replace(/\x1bpy\[(\d+)\]/g, '');
    text = text.replace(/\x1btx\[(\d+)\]/g, '');
    text = text.replace(/\x1bty\[(\d+)\]/g, '');
    text = text.replace(/\x1bop\[(\d+)\]/g, '');
    text = text.replace(/\x1boc\[(\d+)\]/g, '');
    text = text.replace(/\x1bow\[(\d+)\]/g, '');
    text = text.replace(/\x1brc\[(\d+)\]/g, '');
    text = text.replace(/\x1brs\[(\d+)\]/g, '');
    text = text.replace(/\x1brw\[(\d+)\]/g, '');
    text = text.replace(/\x1bdf\[(\d+)\]/g, '');
    text = text.replace(/\x1bsv\[(\d+)\]/g, '');
    text = text.replace(/\x1bld\[(\d+)\]/g, '');

// Convert ruby text
    text = text.replace('\x1brb[Sa,3]\x1brb[mu,6-]\x1brb[rai,Rai] \x1brb[Ya,8]\x1brb[i,1]\x1brb[ba,8]', 'Samurai Yaiba (36-Rai 818)');
    text = text.replace('\x1brb[Sa,3]\x1brb[mu,6-]\x1brb[rai,Rai] \x1brb[On,ON-]\x1brb[na,7]', 'Samurai Onna (36-Rai ON-7)');
    text = text.replace(/\x1brb\[([^,]*),([^\]]*)\]/g, '$1 ($2)');
...
//　　　　　　　　　　ここ▼　　　　　　　　　　　　とここ▼
an_BackLogNowText[0]="##Журнал, страница "+Number(nBackLogPage)+"##";
var j=1;
