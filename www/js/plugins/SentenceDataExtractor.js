...
var mapinfod = fs.readFileSync(projectFilePath+'/'+$dataFolder+'data/MapInfos.json');
var jsonmapd = JSON.parse(mapinfod);

//マップファイルの数だけテキストを作る
for(num=1;num<jsonmapd.length;num++)
{
  var data = fs.readFileSync(projectFilePath+'/'+$dataFolder+'data/Map'+("00"+num).slice(-3)+'.json');
...
//コモンイベントのやつ　ファイル構造が微妙に違うのでまとめられない
  var data = fs.readFileSync(projectFilePath+'/'+$dataFolder+'data/CommonEvents.json');
